...
Сделал: строка ~179, добавил комментарий.
...
const map = {
  ...
  render(snakePointsArray, foodPoint) {
    ...
    this.usedCells.push(foodCell); // тут стояло foodCell #ОШИБКА
  },
};
...
const snake = {
  ...
  getNextStepHeadPoint() {
    ...
    switch (this.direction) {
      case 'up':
        if (firstPoint.y === 0){
            return { x: firstPoint.x, y: settings.colsCount - 1 }
...
const game = {
  ...
  countUp(){
    let points = document.getElementById('counter');
